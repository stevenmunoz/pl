...
class Fairy_Style_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'fairy_style widget-subscribe';
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'fairy_style' );
		$this->widget_id          = 'fairy_style_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'fairy_style' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'fairy_style' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'fairy_style' ),
				'label' => esc_html__( 'Subscribe Title', 'fairy_style' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'fairy_style' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'fairy_style' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'fairy_style' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'fairy_style' ),
				'label' => esc_html__( 'Subscribe submit label', 'fairy_style' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'fairy_style' ),
				'label' => esc_html__( 'Subscribe success', 'fairy_style' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'fairy_style' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'fairy_style' ),
				'label' => esc_html__( 'Follow Title', 'fairy_style' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'fairy_style' ),
			),
		);

		add_action( 'wp_ajax_fairy_style_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_fairy_style_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'fairy_style' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'fairy_style' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'fairy_style' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'fairy_style' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'fairy_style' );
		}
	...
	public function get_social_nav() {
		return fairy_style_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = fairy_style_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'fairy_style_subscribe' ) ) {
...
add_action( 'widgets_init', 'fairy_style_register_subscribe_follow_widgets' );
function fairy_style_register_subscribe_follow_widgets() {
	register_widget( 'Fairy_Style_Subscribe_Follow_Widget' );
}
