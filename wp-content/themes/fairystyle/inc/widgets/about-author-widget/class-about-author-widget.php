...
class Fairy_Style_About_Author_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'fairy_style widget-about-author';
		$this->widget_description = esc_html__( 'Display an information about selected user.', 'fairy_style' );
		$this->widget_id          = 'fairy_style_widget_about_author';
		$this->widget_name        = esc_html__( 'About Author', 'fairy_style' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'value' => esc_html__( 'About Author', 'fairy_style' ),
			...
			'user_id' => array(
				...
				'options'          => false,
				'label'            => esc_html__( 'Select user to show', 'fairy_style' ),
			),
			'avatar_size' => array(
				'type'       => 'slider',
				'max_value'  => 512,
				...
				'value'      => 250,
				'step_value' => 1,
				'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'fairy_style' ),
			),
			'avatar_img' => array(
				'type'               => 'media',
				'value'              => '',
				'multi_upload'       => false,
				'library_type'       => 'image',
				'upload_button_text' => esc_html__( 'Select image', 'fairy_style' ),
				'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'fairy_style' ),
			),
			'link' => array(
				'type'  => 'text',
				'value' => '',
				'label' => esc_html__( 'Link (leave empty to hide)', 'fairy_style' ),
			),
			'link_text' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Read More', 'fairy_style' ),
				'label' => esc_html__( 'Link label', 'fairy_style' ),
			),
		);
	...
	public function get_users_list() {
		...
		$result = array( '0' => __( 'Select a user', 'fairy_style' ) );
...
add_action( 'widgets_init', 'fairy_style_register_about_author_widgets' );
function fairy_style_register_about_author_widgets() {
	register_widget( 'Fairy_Style_About_Author_Widget' );
}
