    ...
    RDMaterialTabs = (function() {
      ...
      * @protected
      ...
      RDMaterialTabs.prototype.setWidth = function(ctx, list) {
        ...
        if (list) {
          ...
          if( item && item.length ) {
            item.each( function() {
              itemsWidth += ( $(this).outerWidth() + margin );
            } );
            item.css({
              'margin-right': ctx.px(margin)
            });
        }
          stage[0].style.width = ctx.px(itemsWidth + padding);
        } else {
        ...
        /*if (ctx.getOption('items') < ctx.$list.find('.' + ctx.settings.itemClass).length) {
          ...
          if( item && item.length ) {
            item.each(function(n){
            	if (n < index) offset -= ($(this).outerWidth() + ctx.getOption('margin'));
            });
          }
      ...
      * @protected
      ...
      RDMaterialTabs.prototype.getMaxTranslate = function(ctx, el) {
        return ctx.getWidth(ctx, el.find('.' + ctx.settings.stageOuterClass)) - ctx.getWidth(ctx, el.find('.' + ctx.settings.stageClass));
      };
