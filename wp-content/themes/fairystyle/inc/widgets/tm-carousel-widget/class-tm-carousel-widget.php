 ...
 * @package __Tm
 */
class Fairy_Style_Carousel_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'fairy_style widget_carousel';
		$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'fairy_style' );
		$this->widget_id          = 'widget_carousel';
		$this->widget_name        = esc_html__( 'Carousel Widget', 'fairy_style' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Carousel', 'fairy_style' ),
			...
			'slides_per_view' => array(
				...
				'min_value' => 1,
				'value'     => 5,
				'label'     => esc_html__( 'Number of slides per view', 'fairy_style' ),
			),
			'slides_per_group' => array(
				...
				'min_value' => 1,
				'value'     => 1,
				'label'     => esc_html__( 'Number slides per group', 'fairy_style' ),
			),
			'slides_per_column' => array(
				...
				'min_value' => 1,
				'value'     => 1,
				'label'     => esc_html__( 'Multi Row Slides Layout', 'fairy_style' ),
			),
			'space_between_slides' => array(
				...
				'min_value' => 0,
				'value'     => 30,
				'label'     => esc_html__( 'Width of the space between slides(px)', 'fairy_style' ),
			),
			'duration_speed' => array(
				...
				'min_value' => 100,
				'value'     => 500,
				'label'     => esc_html__( 'Duration of transition between slides (ms)', 'fairy_style' ),
			),
			'navigation' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Slider navigation', 'fairy_style' ),
			),
			'pagination' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Slider pagination', 'fairy_style' ),
			),
		);
	...
	public function widget( $args, $instance ) {
		...
		$footer_widget_columns = get_theme_mod( 'footer_widget_columns', fairy_style_theme()->customizer->get_default( 'footer_widget_columns' ) );
		...
		$data_attr_line = 'class="fairy_style-carousel swiper-container"';
		...
		if ( $posts_query ) {
			...
			echo '<h5>' . esc_html__( 'Posts not found', 'fairy_style' ) . '</h5>';
		}
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'fairy_style_carousel_default_query_args', array(
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
			'posts_per_page' => -1,
	...
	public function get_carousel_loop( $posts_query ) {
		$image_size = apply_filters( 'fairy_style_carousel_image_size',
			'fairy_style-thumb-370-270',
			$this->instance,
			$this->args
		);
		...
		if ( ! $view_dir ) {
			return '<h5>' . esc_html__( 'View file not found', 'fairy_style' ) . '</h5>';
		}
		...
		while ( $posts_query->have_posts() ) : $posts_query->the_post();
			...
			$placeholder_args = apply_filters( 'fairy_style_carousel_placeholder_args', array(
				'width'      => 560,
				'height'     => 350,
				'background' => '000',
				'foreground' => 'fff',
	...
	public function get_image( $id, $size, $placeholder_attr, $only_url = false ) {
		...
		if ( has_post_thumbnail( $id ) ) {
			...
			$default_placeholder_attr = apply_filters( 'fairy_style_carousel_placeholder_default_args', array(
				'width'      => 900,
				'height'     => 500,
				'background' => '000',
				'foreground' => 'fff',
	...
	public function get_post_comments( $post_id ) {
		...
		$title_comments =  sprintf( _n( '%s' . __( " Comment", "fairy_style" ) , '%s'. __( " Comments", "fairy_style" ), $comments, 'fairy_style' ), $comments );
		$comments = ( ! empty( $comments ) ) ? sprintf( '<span class="post-comments-link"><a href="%1$s">%2$s</a></span>', esc_url( get_comments_link() ), $title_comments ) : esc_html__( '0 Comments', 'fairy_style' );
	...
	public function get_post_date( $post_id ) {
		 ...
		 * @var string
		 */
		$date_format = apply_filters( 'fairy_style_carousel_post_dateformat', 'M d, Y', $this->instance );
...
function tm_register_carosel_widgets() {
	register_widget( 'Fairy_Style_Carousel_Widget' );
}