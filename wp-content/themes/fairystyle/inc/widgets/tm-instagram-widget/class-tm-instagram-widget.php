 ...
 * @package Gadnews
 */
if ( ! class_exists( 'Gadnews_Instagram_Widget' ) ) {
	class Gadnews_Instagram_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_name        = esc_html__( 'Instagram', 'gadnews' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
		...
		public function widget( $args, $instance ) {
			...
			if ( empty( $instance['tag'] ) ) {
				return print $args['before_widget'] . esc_html__( 'Please, enter #hashtag.', 'gadnews' ) . $args['after_widget'];
			}
			ob_start();
			...
			if ( ! $photos ) {
				return print $args['before_widget'] . esc_html__( 'No photos. Maybe you entered a invalid hashtag.', 'gadnews' ) . $args['after_widget'];
			}
		...
		public function get_photos( $data, $img_counter, $config ) {
			 ...
			 * @var string
			 */
			$order = apply_filters( 'gadnews_instagram_widget_order', 'media' );
			...
			foreach ( $nodes as $photo ) {
				...
				if ( in_array( 'caption', $config ) ) {
					$_photo['caption'] = wp_html_excerpt(
						...
						apply_filters( 'gadnews_instagram_widget_caption_more', '&hellip;' )
					);
				}
		...
		public function get_tags_url() {
			return apply_filters( 'gadnews_instagram_widget_get_tags_url', $this->service_url . 'explore/tags/%s/' );
		}
		/**
		...
		public function get_post_url() {
			return apply_filters( 'gadnews_instagram_widget_get_post_url', $this->service_url . 'p/%s/' );
		}
		/**
		 * Clear cache.
		 *
