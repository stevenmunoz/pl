(function($){
	...
	CherryJsCore.theme_script = {
		...
		smart_slider_init: function( self ) {
			$( '.fairy_style-smartslider' ).each( function() {
				var slider = $(this),
				...
				if ( $('.fairy_style-smartslider__slides', '#' + sliderId ).length > 0 ) {
					$( '#' + sliderId ).sliderPro( {
						width: sliderWidth,
						height: sliderHeight,
		...
		swiper_carousel_init: function ( self ) {
			...
			jQuery('.fairy_style-carousel').each( function() {
				var swiper = null,
		...
		subscribe_init: function( self ) {
			CherryJsCore.variable.$document.on( 'click', '.subscribe-block__submit', function( event ){
				...
				var $this       = $(this),
					...
					nonce      = form.find( 'input[name="fairy_style_subscribe"]' ).val(),
				...
				$.ajax({
					url: fairy_style.ajaxurl,
					type: 'post',
					dataType: 'json',
					data: {
						action: 'fairy_style_subscribe',
						mail: mail,
						nonce: nonce
					},
					error: function() {
		...
		to_top_init: function ( self ) {
			if ( $.isFunction( jQuery.fn.UItoTop ) ) {
				$().UItoTop({
					text: fairy_style.labels.totop_button,
					scrollSpeed: 600
				});
			}
		}
...
// Dropdown toggle
	...
	$(document).on('click', '.cherry-compare', function (ev) {
		...
		var table_url = this.href;
		 $('body').trigger('yith_woocompare_open_popup', {response: fairy_style.compare_table_url, button: $(this)});
	});
