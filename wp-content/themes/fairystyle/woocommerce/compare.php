...
<table class="compare-list products" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
	<thead>
	<tr>
		<th>&nbsp;</th>
	...
	<?php else : ?>
		...
		<?php foreach ( $fields as $field => $name ) : ?>
			...
			<tr class="<?php echo $field ?>">
				...
				<?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id; ?>
					<td class="product <?php echo $product_class; ?>"><?php
						switch( $field ) {
							...
							case 'title':
								echo empty( $product->fields[$field] ) ? '&nbsp;' : '<h3>' . $product->fields[$field] . '</h3>';
