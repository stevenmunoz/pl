...
function snp_builder_create_element($type, $step_index, $el_index, $args)
{
    ...
    if ($type == 'text') {
        ...
        if (isset($args['mailchimp_group']) && !empty($args['mailchimp_group'])) {
            ...
            foreach ($fields->interests as $v) {
                $return .= '<label>';
                $return .= '<input type="checkbox" '.$data.' value="' . $v->id . '" class="bld-el '.$css_class.'"'.$tabindex.' />';
                $return .= '<span>' . $v->name . '</span>';
                $return .= '</label>';
                $return .= '<span class="snp_clearfix"></span>';
            }
            ...
            $return .= '<label>';
            $return .= '<input type="checkbox" '.$data.'  '.(isset($args['value']) ? 'value="'.$args['value'].'"' : '').'  class="bld-el '.$css_class.'"'.$tabindex.' />';
            $return .= '<span>' . $args['label'] . '</span>';
            $return .= '</label>';
        }
        $return .= '</div>';
        ...
        $return .= '<div class="bld-table-cont">';
        $return .= '<input  '.$data.'  '.(isset($args['text']) ? 'value="'.$args['text'].'"' : '').'  class="bld-el snp-calendar-input '.$css_class.'"'.$tabindex.' />';
        $return .= '</div>';
    } else if ($type == 'hidden') {
        $return .= '<div class="bld-table-cont">';
